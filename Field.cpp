...
bool CField::AHZIsInOurPenaltyArea(Vector2D point)
{
    Circle2D c1(fOurGoal + Vector2D(0,-_GOAL_WIDTH/4),_AHZ_GOAL_RAD_FOR_GOALKEEPER);
    Circle2D c2(fOurGoal + Vector2D(0,+_GOAL_WIDTH/4), _AHZ_GOAL_RAD_FOR_GOALKEEPER);
...
Rect2D   CField::getRegion(Region region, double k)
{
    ...
    case LargeOppCornerBottomLeft : tmp = getRegion(LargeOppCornerBottom);tmp.setLength(tmp.size().length()*0.5);return tmp;
...
double CField::ourPAreaPos(double angle)
{
    ...
    else if (p.y > -_GOAL_WIDTH/4 && p.y < _GOAL_WIDTH/4 )
    {
        return p.y;
    }
    else if (p.y > _GOAL_WIDTH/4)
    {
        return _GOAL_WIDTH/4 + fabs(Vector2D::angleBetween(n, Vector2D(0.0, 0.0)).radian()) * 0.50;
    }
}
