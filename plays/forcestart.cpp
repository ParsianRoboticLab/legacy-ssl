...
CForceStart::CForceStart()
{
    ...
    kkARegion[0].assign(-_FIELD_WIDTH/2,                     //top_X
                        _FIELD_HEIGHT/2,                     //top_Y
                        _FIELD_WIDTH/2 - _CENTER_CIRCLE_RAD, //X_Length
                        _FIELD_HEIGHT);                      //Y_Length

    kkARegion[1].assign(-_CENTER_CIRCLE_RAD,                    //top_X
                        _FIELD_HEIGHT/2,                        //top_Y
                        _FIELD_WIDTH/2  + _CENTER_CIRCLE_RAD,   //X_Length
                        _FIELD_HEIGHT/2 - (_PENALTY_WIDTH)/2);  //Y_Length

    kkARegion[2].assign(-_CENTER_CIRCLE_RAD,                              //top_X
                        _PENALTY_WIDTH/2,                                 //top_Y
                        _FIELD_WIDTH/2  + _CENTER_CIRCLE_RAD - _GOAL_RAD, //X_Length
                        _PENALTY_WIDTH);                                  //Y_Length

    kkARegion[3].assign(_FIELD_WIDTH/2 - _GOAL_RAD, //top_X
                        _PENALTY_WIDTH/2,           //top_Y
                        _GOAL_RAD,                  //X_Length
                        _PENALTY_WIDTH);            //Y_Length

    kkARegion[4].assign(-_CENTER_CIRCLE_RAD,                    //top_X
                        -_PENALTY_WIDTH/2,                      //top_Y
                        _FIELD_WIDTH/2  + _CENTER_CIRCLE_RAD,   //X_Length
                        _FIELD_HEIGHT/2 - (_PENALTY_WIDTH)/2);  //Y_Length

    for(size_t i = 0 ; i < 5 ; i++) {
        kkBRegion[i] = kkARegion[i];
    }
...
//    }
    ...
    SStaticPlan tempPlan;

        tempPlan.ballRegion = -1;
        tempPlan.agentSize = -1;
        for(size_t i = 0;i < 5;i++) {
            tempPlan.agentsRegion[i] = 0;
        }
    ...
    for(size_t i = 0;i < 5;i++) { //Ball Regions
        tempPlan.ballRegion = i;
        for(size_t j = 0;j < kk1AgentPlan[i]->size();j++) {//Number of 1Agent Plans
            tempPlan.agentSize = 1;
            for(size_t l = 0;l < 5;l++) { //agents Regions
                for(size_t k = 0;k < tempPlan.agentSize;k++) { //agentsCounts
                    if(kk1AgentPlan[i]->at(j).agents[k].A == l) {
                        tempPlan.agentsRegion[l]++;
                    }
                }
            }
            staticPlans.append(tempPlan);
            tempPlan.clean();
        }
        for(size_t j = 0;j < kk2AgentPlan[i]->size();j++) {//Number of 2Agent Plans
            tempPlan.agentSize = 2;
            for(size_t l = 0;l < 5;l++) { //agents Regions
                for(size_t k = 0;k < tempPlan.agentSize;k++) { //agentsCounts
                    if(kk2AgentPlan[i]->at(j).agents[k].A == l) {
                        tempPlan.agentsRegion[l]++;
                    }
                }
            }
        ...
        for(size_t j = 0;j < kk3AgentPlan[i]->size();j++) {//Number of 3Agent Plans
            tempPlan.agentSize = 3;
            for(size_t l = 0;l < 5;l++) { //agents Regions
                for(size_t k = 0;k < tempPlan.agentSize;k++) { //agentsCounts
                    if(kk3AgentPlan[i]->at(j).agents[k].A == l) {
                        tempPlan.agentsRegion[l]++;
                    }
                }
            }
        ...
        for(size_t j = 0;j < kk4AgentPlan[i]->size();j++) {//Number of 4Agent Plans
            tempPlan.agentSize = 4;
            for(size_t l = 0;l < 5;l++) { //agents Regions
                for(size_t k = 0;k < tempPlan.agentSize;k++) { //agentsCounts
                    if(kk4AgentPlan[i]->at(j).agents[k].A == l) {
                        tempPlan.agentsRegion[l]++;
                    }
                }
            }
        ...
        for(size_t j = 0;j < kk5AgentPlan[i]->size();j++) {//Number of 5Agent Plans
            tempPlan.agentSize = 5;
            for(size_t l = 0;l < 5;l++) { //agents Regions
                for(size_t k = 0;k < tempPlan.agentSize;k++) { //agentsCounts
                    if(kk5AgentPlan[i]->at(j).agents[k].A == l) {
                        tempPlan.agentsRegion[l]++;
                    }
                }
            }
    ...
    ////////////////////////////////// AHZ ///////////////////////
    failedTasks.append(KickOffensive);
    failedTasks.append(KickDefensive);
    failedTasks.append(PassOffensive);
    failedTasks.append(PassDefensive);
    failedTasks.append(ChipOffensive);
    failedTasks.append(ChipDefensive);
    failedTasks.append(ReceivePass);
    failedTasks.append(OneTouch);
    failedTasks.append(Shot);
    failedTasks.append(ChipToGoal);

    doneTasks.append(ReceivePass);
    doneTasks.append(OneTouch);
    doneTasks.append(Shot);
    doneTasks.append(ChipToGoal);
}
...
bool CForceStart::kkIsAgentTaskFailed(int tAgent) {
    ...
    Vector2D currentRobotPosition = knowledge->getAgent(kkAgentsId[tAgent])->pos();
    Vector2D targetRobotPosition = agentSkill[tAgent]->getTarget();
    int AHZTime = (pow((currentRobotPosition - targetRobotPosition).x,2) +  pow((currentRobotPosition - targetRobotPosition).y,2)) / (pow(ballVel.x,2) +  pow(ballVel.y,2));

    if(kkIsAgentTaskDone(tAgent)) {
        //debug(QString("Done"),D_SEPEHR);
        return false;
    }
    ...
    case PassOffensive:
        ...
        if(Circle2D((knowledge->getAgent(kkAgentsId[tAgent])->dir()).norm()*0.09
                    + knowledge->getAgent(kkAgentsId[tAgent])->pos(),0.3)
                .contains(ballPos)) {

            ATFballEnteredKickerFlag = true;
        }
        else if(AHZTime > 30){
            debug(QString("Time out / failed"),D_SEPEHR);
            return true;
        }
        draw(Segment2D(ballPos,ballPos + ballVel * 10),QColor(Qt::black));
        // draw(Circle2D(agentSkill[tAgent]->getTarget(),1),QColor(Qt::black));
        if(ATFballEnteredKickerFlag){
            //debug(QString("flag is OK"),D_SEPEHR);
        }
        if(!Circle2D(knowledge->getAgent(kkAgentsId[tAgent])->pos(),0.4).contains(ballPos + ballVel)){
            //debug(QString("circle is OK"),D_SEPEHR);
        }
        if((!(Circle2D(knowledge->getAgent(kkAgentsId[tAgent])->pos(),0.4).contains(ballPos + ballVel))) && ATFballEnteredKickerFlag){
            //debug(QString("OK"),D_SEPEHR);
        }

        if(!Circle2D(knowledge->getAgent(kkAgentsId[tAgent])->pos(),0.5).contains(ballPos + ballVel) && ATFballEnteredKickerFlag) {
            debug(QString("test"),D_SEPEHR);
            draw(Circle2D(agentSkill[tAgent]->getTarget(),1),QColor(Qt::black));
            draw(Segment2D(ballPos,ballPos + ballVel * 10),QColor(Qt::black));
            if(Circle2D(agentSkill[tAgent]->getTarget(),1).intersection(Segment2D(ballPos,ballPos + ballVel * 10),&sol1,&sol2)) {
                debug(QString("YES"),D_SEPEHR);
                return false;
            }
            //            else {
            //                ATFballEnteredKickerFlag = false;
    ...
    case ChipDefensive:
        ...
        if(!Circle2D(knowledge->getAgent(kkAgentsId[tAgent])->pos(),1).contains(ballPos) && ATFballEnteredKickerChipFlag) {
            if(Circle2D(agentSkill[tAgent]->getTarget(),0.5).intersection(Ray2D(ballPos,ballPos + ballVel),&sol1,&sol2)) return false;
                ...
                ATFballEnteredKickerChipFlag = false;
                ATFpassReceivedFlag = false;
                debug(QString("chipoffense failed"),D_SEPEHR);
                return true;
            }
        }
        else return false;
    ...
    case OneTouch:
        ...
        if(Circle2D(agentSkill[tAgent]->getWaitPos(),0.4).contains(ballPos)) {
            ATFpassReceivedFlag = true;
        }
        ////////////////////////////////// Wrong Condition ////////////
        if(ATFpassReceivedFlag && !Circle2D(knowledge->getAgent(kkAgentsId[tAgent])->pos(),1).contains(ballPos)){
            ATFballEnteredKickerFlag = false;
            ATFballEnteredKickerChipFlag = false;
            ATFpassReceivedFlag = false;
            debug(QString("onetouch failed"),D_SEPEHR);
            return true;
        }
        ...
        if(!Circle2D(knowledge->getAgent(kkAgentsId[tAgent])->pos(),1).contains(ballPos) && ATFballEnteredKickerChipFlag) {
            draw(Segment2D(ballPos,ballPos + ballVel * 10),QColor(Qt::black));
            if(Circle2D(agentSkill[tAgent]->getTarget(),2).intersection(Segment2D(ballPos,ballPos + ballVel * 10),&sol1,&sol2)) return false;
            else {
                ATFballEnteredKickerFlag = false;
                ATFballEnteredKickerChipFlag = false;
                debug(QString("touch failed"),D_SEPEHR);
                return true;
            }
        }
        else return false;
    ...
    case ReceivePass:
        ...
        if(Circle2D(agentSkill[tAgent]->getTarget(),0.5).contains(ballPos)) {
            ATFpassReceivedFlag = true;
        }
        if(ATFpassReceivedFlag && !Circle2D(knowledge->getAgent(kkAgentsId[tAgent])->pos(),0.35).contains(ballPos)){
            ATFpassReceivedFlag = false;
            debug(QString("receive failed"),D_SEPEHR);
            return true;
        }
        break;
    case Shot:
        ...
        if(!Circle2D(knowledge->getAgent(kkAgentsId[tAgent])->pos(),1).contains(ballPos) && ATFballEnteredKickerFlag) {
            if(Circle2D(agentSkill[tAgent]->getTarget(),0.5).intersection(Ray2D(ballPos,ballPos + ballVel),&sol1,&sol2)) return false;
                ...
                ATFballEnteredKickerChipFlag = false;
                ATFpassReceivedFlag = false;
                debug(QString("shot failed"),D_SEPEHR);
                return true;
            }
        }
        else return false;
    ...
    case ChipToGoal:
        ...
        if(!Circle2D(knowledge->getAgent(kkAgentsId[tAgent])->pos(),1).contains(ballPos) && ATFballEnteredKickerFlag) {
            if(Circle2D(agentSkill[tAgent]->getTarget(),0.5).intersection(Ray2D(ballPos,ballPos + ballVel),&sol1,&sol2)) return false;
                ...
                ATFballEnteredKickerChipFlag = false;
                ATFpassReceivedFlag = false;
                debug(QString("chiptogoal failed"),D_SEPEHR);
                return true;
            }
        }
        else return false;
...
{
    Vector2D ballPos = wm->ball->pos;
    Vector2D ballVel = wm->ball->pos;
    switch(agentSkill[tAgent]->getTask())
    {
    case MoveOffensive:
    case MoveDefensive:
        if(Circle2D(agentSkill[tAgent]->getTarget(), 2).contains(knowledge->getAgent(agentsID[tAgent])->pos())) {
            //  debug(QString("move done"),D_SEPEHR);
            return true;
        }
        else
            return false;
    ...
    case PassDefensive:
        ...
        //draw(Circle2D(agentSkill[tAgent]->getTarget(), 0.4),QColor(Qt::black));
        if(Circle2D(agentSkill[tAgent]->getTarget(), 0.4).contains(ballPos + ballVel)) {
            debug(QString("pass done"),D_SEPEHR);
            return true;
        }
        else {
            return false;
    ...
    case KickDefensive:
        ...
        //draw(Circle2D(agentSkill[tAgent]->getTarget(), 0.4),QColor(Qt::black));
        if(Circle2D(agentSkill[tAgent]->getTarget(), 0.4).contains(ballPos + ballVel)){
            debug(QString("kick done"),D_SEPEHR);
            return true;
        }
        else
            return false;
    ...
    case ChipDefensive:
        ...
        if(Circle2D(agentSkill[tAgent]->getTarget(), 0.3).contains(wm->ball->pos)){
            debug(QString("chipoffens done"),D_SEPEHR);
            return true;
        }
        else
            return false;
        break;
    case OneTouch:
        draw((Circle2D(agentSkill[tAgent]->getWaitPos(), 0.8)),QColor(Qt::black));
        if(Circle2D(agentSkill[tAgent]->getWaitPos(), 0.8).contains(ballPos)) {
            kkTaskDoneOneTouchEnterFlag = true;
            kkTaskDoneRadiusTolerance = 0.3;
            debug(QString("tolerance"),D_SEPEHR);
        }
        ...
        //////////////////////// Wrong Condition //////////////////////////////////
            ...
            kkTaskDoneOneTouchEnterFlag = false;
            //kkTaskDoneRadiusTolerance = 0.0;
            //debug(QString("not tolerance"),D_SEPEHR);
        }



        //draw(Circle2D((knowledge->getAgent(kkAgentsId[tAgent])->dir()).norm() * 0.1 + knowledge->getAgent(kkAgentsId[tAgent])->pos(),0.08),QColor(Qt::red));
        if(Circle2D((knowledge->getAgent(kkAgentsId[tAgent])->dir()).norm()*0.1  + knowledge->getAgent(kkAgentsId[tAgent])->pos(),0.08).contains(ballPos)){

            kkTaskDoneOneTouchEnterFlag1 = true;
        }
        if(kkTaskDoneOneTouchEnterFlag && kkTaskDoneOneTouchEnterFlag1) {
            debug(QString(" && "),D_SEPEHR);
            if((Circle2D(agentSkill[tAgent]->getWaitPos(),0.5 + kkTaskDoneRadiusTolerance).contains(ballPos))) {
                ...
                debug(QString("onetouch done"),D_SEPEHR);
                return true;
            }
        }
        else return false;
    ...
    case CatchBall:
        ...
        if(Circle2D(ballPos, 0.2).contains(knowledge->getAgent(kkAgentsId[tAgent])->pos()) ){
            debug(QString("catchball done"),D_SEPEHR);
            return true;
        }
        return false;
        break;
    case ReceivePass:
        ...
        if(Circle2D(knowledge->getAgent(kkAgentsId[tAgent])->pos(), 0.55).contains(ballPos)){
            debug(QString("receive done"),D_SEPEHR);
            return true;
        }
        else{
            return false;
    ...
    case ChipToGoal:
        ...
        if(wm->field->isInOppPenaltyArea(ballPos)){
            debug(QString("chiptogoal done"),D_SEPEHR);
            return true;
        }
        else{
            return false;
...
{
    int tempDoneCounter = 0;
    int tempFailedCounter = 0;


    //    draw(Circle2D(agentSkill[tAgent]->getTarget(),3),QColor(Qt::black));

    for(int i = 0 ; i < kkCurrentPlan.agentsSize;i++){
        if(failedTasks.contains(agentSkill[i]->getTask())){
            if(kkIsAgentTaskFailed(i)){
                debug(QString("One Failed"),D_SEPEHR);
                decidePlan = true;
                return;
            }
        }
        if(doneTasks.contains(agentSkill[i]->getTask())){
            if(kkIsAgentTaskDone(i)){
                debug(QString("One Done"),D_SEPEHR);
                decidePlan = true;
                return;
            }
        }
    ...
    ////////////////////// All Agents /////////////////////////////////////////
    /*check if the exact  agent plan Failed or Done */
    for(int i = 0; i < kkCurrentPlan.agentsSize; i++)
    {
        if(!kkDoneTaskAgent[i])
            kkDoneTaskAgent[i] = kkIsAgentTaskDone(i);
        else if(!kkFailedTaskAgent[i])
            kkFailedTaskAgent[i] = kkIsAgentTaskFailed(i);
    }
    /*check if the sum Failed and Done is equal with inPr`ogress*/
    for(int i = 0; i < kkCurrentPlan.agentsSize; i++){
        if(kkDoneTaskAgent[i])
            tempDoneCounter++;
        else if(kkFailedTaskAgent[i])
    ...
    if(tempDoneCounter + tempFailedCounter >= kkCurrentPlan.agentsSize)
    {
        decidePlan = true;
        debug(QString("Allagent"),D_SEPEHR);
        return;
    }
    //////////////////////////////////// Cycle ////////////////////////////////
...
///////////////////////////Dynamic Attack
/////////////////////////////////////////
//void CForceStart::dynamicAttack(int agentSize) {


//}

//SDynamicPlan CForceStart::isDynamic(int agentSize) {
//    //Agent size is always more than zero.
//    SDynamicPlan temp();
//    Vector2D ballPos = wm->ball->pos;


//    double prob[2],angle[2],biggestAngle[2];
//    QList <Circle2D> obstacles;

//    for(int i = 0 ; i < wm->opp.activeAgentsCount() ; i++)
//        obstacles.append(Circle2D(wm->opp.active(i)->pos,0.1));

//    knowledge->getEmptyAngle(wm->ball->pos -
//                             (wm->field->oppGoal() - ballPos).norm()*0.15,
//                             wm->field->oppGoalL(),wm->field->oppGoalR(),
//                             obstacles, prob[0],
//                             angle[0], biggestAngle[0]);


//    debug(QString("prob: %1 , angle :%2, biggest:%3")
//          .arg(prob[0])
//          .arg(angle[0])
//          .arg(biggestAngle[0])
//          ,D_MAHI);
...
//    if(agentSize == 1) {
//        if(prob[0] > 0.85) {
//            temp.mode  = DynamicOneShot;
//            temp.dir   = angle;
//            temp.angle = biggestAngle;
//            return temp;
//        }
//    }
//    else {
//        for(size_t i = 0;i < agentSize;i++) {
//            if(kkActiveAgents.at(i)->pos().x >= 1) {
//                knowledge->getEmptyAngle(kkActiveAgents.at(i)->pos(),
//                                         wm->field->oppGoalL(),wm->field->oppGoalR(),
//                                         obstacles, prob[1],
//                                         angle[1], biggestAngle[1]);
//            }
//        }
//    }
...
void CForceStart::kkMainPlanner(int tAgentSize)
{
    ...
    debug(QString("plan id : %1").arg(kkCurrentPlan.planId),D_MAHI);
    debug(QString("is ball ours : %1").arg(kkCurrentPlan.ball),D_MAHI);
    ...
    if(decidePlan)
    {
        ...
        int tempBall = kkGetArea(kkBRegion, 5, wm->ball->pos);
        if(!kkGetBestPlan(tempBall, tAgentSize)) {
            decidePlan      = true;
            lockAgents      = false;
            noPlanException = true;
            debug(QString("No Plan Exception : agentSize = %1 , ballPos = %2")
                  .arg(tAgentSize).arg(tempBall),D_MAHI);
            return;
        }
        mahiAssignId();
        kkNewAssignTasks();
    ...
    if(!kkMarkList.isEmpty() && wm->opp.activeAgentsCount())
    {
        int tempId[3];
    ...
    mahiDebugs();
    if(decidePlan) lockAgents = false;

}
void CForceStart::execute_0(){
    draw("KKPLayOn agent(s): 0",
         Vector2D(_FIELD_WIDTH/2 - 1.5, _FIELD_HEIGHT/2 + 0.15), QColor(0, 51, 102));
    if(lastAgentCount != 0) decidePlan = true;
    lastAgentCount = 0;
}

void CForceStart::execute_1(){
    draw("KKPLayOn agent(s): 1",
         Vector2D(_FIELD_WIDTH/2 - 1.5, _FIELD_HEIGHT/2 + 0.15), QColor(0, 51, 102));
...
void CForceStart::kkInsertToList(QList<kkAgentPlan> **list, QSqlQuery query,
                                 int agentSize)
{
    ...
    while(query.next())
    {
        ...
        kkAgentPlan tempSymmetryPlan;

        tempPlan.planId     = 2*(query.value(0).toInt());
        ...
        tempPlan.comment    = query.value(6).toString();

        //////////////////////////////////
        //////////////SYMMETRY////////////
        //////////////////////////////////
        tempSymmetryPlan.planId     = 2*(query.value(0).toInt()) + 1;
        tempSymmetryPlan.ball       = getSymmetry(query.value(1).toInt());
        tempSymmetryPlan.endMode    = kkGetPolicyByIndex(query.value(2).toInt());
        tempSymmetryPlan.endPolicy  = query.value(3).toInt();
        tempSymmetryPlan.possession = query.value(4).toInt();
        tempSymmetryPlan.agentsSize = query.value(5).toInt();
        tempSymmetryPlan.comment    = query.value(6).toString();
        ...
        for(int i = 0; i < agentSize; i++)
        {
            ...
            for(int j = 0; j < 4; j++)
            {
                tempPlan.agents[i].skill[j] =
                        kkGetPSkillByIndex(query.value(9 + i*6 + j).toInt());
            }
        }
        ...
        for(int i = 0; i < agentSize; i++)
        {
            tempSymmetryPlan.agents[i].A =
                    getSymmetry(query.value(7 + i*6).toInt());
            tempSymmetryPlan.agents[i].B =
                    getSymmetry(query.value(8 + i*6).toInt());
            for(int j = 0; j < 4; j++)
            {
                tempSymmetryPlan.agents[i].skill[j] =
                        kkGetPSkillByIndex(query.value(9 + i*6 + j).toInt());
            }
        }
        for(int i = agentSize; i < 5; i++)
        {
            tempSymmetryPlan.agents[i].A = -1;
            tempSymmetryPlan.agents[i].B = -1;
            for(int j = 0; j < 4; j++)
            {
                tempSymmetryPlan.agents[i].skill[j] = None;
            }
        }
        ...
        switch(tempSymmetryPlan.ball)
        {
        default:
        case 1:
            list[0]->append(tempSymmetryPlan);
            break;
        case 2:
            list[1]->append(tempSymmetryPlan);
            break;
        case 3:
            list[2]->append(tempSymmetryPlan);
            break;
        case 4:
            list[3]->append(tempSymmetryPlan);
            break;
        case 5:
            list[4]->append(tempSymmetryPlan);
            break;
        case 6:
            list[5]->append(tempSymmetryPlan);
            break;
        }
...
{
    Rect2D tempRect;
    tempRect.assign(rect.left() - offset, rect.top() + offset,
                    rect.size().length() + offset*2, rect.size().width() + offset*2);
...
bool CForceStart::kkGetBestPlan(int planBallPos, int agentSize)
{
    QList<kkAgentPlan> tempPlans;
    planBallPos--;
    ...
    default:
    case 1:
        tempPlans = kkFillQList(kk1AgentPlan[planBallPos]);
        break;
    case 2:
        tempPlans = kkFillQList(kk2AgentPlan[planBallPos]);
        break;
    case 3:
        tempPlans = kkFillQList(kk3AgentPlan[planBallPos]);
        break;
    case 4:
        tempPlans = kkFillQList(kk4AgentPlan[planBallPos]);
        break;
    case 5:
        tempPlans = kkFillQList(kk5AgentPlan[planBallPos]);
        break;
    }
    ...
    if(tempPlans.isEmpty()) {
        ...
        for(int i = 0; i < tempPlans.length(); i++)
        {
            if(kkIsPlanMatch(kkAgentList, tempPlans.at(i))) {
                selectedPlans.append(i);
            }
        }
        debug(QString("Matched plan(s): %1/%2 agentSize: %3").
              arg(selectedPlans.length()).arg(tempPlans.length()).arg(agentSize), D_KK);

        if(selectedPlans.length() == 1)
        {
            kkCurrentPlan = tempPlans.at(selectedPlans.at(0));
        }
        else if(selectedPlans.length() > 1)
        {
            QList<kkAgentPlan> tempExtraPlan;
            for(int i = 0; i < selectedPlans.length(); i++)
                tempExtraPlan.append(tempPlans.at(selectedPlans.at(i)));
...
//        else if(selectedPlans.length() == 0 && tempPlans.length() != 0)
//        {
//            kkCurrentPlan = kkChooseBetweenInappropiatePlan(tempPlans);
//        }

        debug(QString("%1, %2, %3").
              arg(selectedPlans.length()).
              arg(agentSize).
              arg(tempPlans.length()), D_KK);
        noPlanException = false;
        return true;
    }
}
...
kkAgentPlan CForceStart::kkChooseBetweenInappropiatePlan(QList<kkAgentPlan> tList)
{
    ...
    for(int k = 0; k < tList.length(); k++)
    {
        ...
        for(int i = 0; i < tList.at(k).agentsSize; i++)
        {
            ...
            {
                distPoint[i][j] = kkARegion[tList.at(k).agents[i].A - 1].
                        center().dist(kkAgentList.at(j).pos);
            }
        }
        ...
        kkGetCostRec(distPoint, tList.at(k).agentsSize,
                     tempListValue, tempValue,
                     tList.at(k).agentsSize, 0);
...
void CForceStart::mahiAssignId() {

    int firstBallAgent = -1;
    int secondBallAgent = -1;

    QList<PSkills> firstBallSkills;
    QList<PSkills> secondBallSkills;
    QList<int> matchedID;
    ...
    firstBallSkills.append(ChipDefensive);
    firstBallSkills.append(ChipOffensive);
    firstBallSkills.append(PassOffensive);
    firstBallSkills.append(PassDefensive);
    firstBallSkills.append(KickOffensive);
    firstBallSkills.append(KickDefensive);
    firstBallSkills.append(Shot);
    firstBallSkills.append(ChipToGoal);
    firstBallSkills.append(CatchBall);

    secondBallSkills.append(ReceivePass);
    secondBallSkills.append(OneTouch);

    for(size_t i = 0;i < kkCurrentPlan.agentsSize;i++) {
        if(firstBallSkills.contains(kkCurrentPlan.agents[i].skill[0])) {
            firstBallAgent = i;
        }
        else if(secondBallSkills.contains(kkCurrentPlan.agents[i].skill[0])) {
            secondBallAgent = i;
        }
    }
    ...
    if(firstBallAgent >= 0) {
        for(size_t i = 0;i < kkActiveAgents.size();i++) {
            dist2Point[i] = kkActiveAgents.at(i)->pos()
                    .dist(wm->ball->pos);
            if(dist2Point[i] < minDist) {
                minDist = dist2Point[i];
                kkAgentsId[firstBallAgent] = kkActiveAgents.at(i)->id();
            }
        }
        matchedID.append(kkAgentsId[firstBallAgent]);
    }
    ...
    if(secondBallAgent >= 0) {
        for(size_t i = 0;i < kkActiveAgents.size();i++) {
            if(!matchedID.contains(kkActiveAgents.at(i)->id())) {
                dist2Point[i] = kkActiveAgents.at(i)->pos()
                        .dist(kkARegion[kkCurrentPlan.agents[secondBallAgent].B - 1].center());
                if(dist2Point[i] < minDist) {
                    minDist = dist2Point[i];
                    kkAgentsId[secondBallAgent] = kkActiveAgents.at(i)->id();
                }
            }
        }
        matchedID.append(kkAgentsId[secondBallAgent]);
    }

    for(size_t i = 0;i < kkCurrentPlan.agentsSize;i++) {

        if(i != firstBallAgent && i != secondBallAgent) {
            ...
            for(size_t j = 0; j < kkActiveAgents.size();j++) {
                if(!matchedID.contains(kkActiveAgents.at(j)->id())) {
                    dist2Point[i] = kkARegion[kkCurrentPlan.agents[i].A - 1].center().dist(kkActiveAgents.at(j)->pos());
                    if(dist2Point[i] < minDist) {
                        minDist = dist2Point[i];
                        kkAgentsId[i] = kkActiveAgents.at(j)->id();
                    }
                }
            }
            matchedID.append(kkAgentsId[i]);
        }

        //if(firstBallAgent >= 0)
        //debugs[0] = kkAgentsId[firstBallAgent];
        // else
        //debugs[0] = -1;
        if(secondBallAgent >= 0)
            debugs[1] = kkAgentsId[secondBallAgent];
        else
            debugs[1] = -1;
    }
}
...
void CForceStart::kkNewAssignId() {
    ...
    for(int i = 0; i < kkCurrentPlan.agentsSize; i++){
        if(ballDependenceSkills.contains(kkCurrentPlan.agents[i].skill[0]))
            ballDependenceSkill.append(i);
    ...
    if((ballDependenceSkill.size()) &&
            cntRobotInRegion(kkARegion[kkCurrentPlan.
                             agents[ballDependenceSkill.at(0)].A - 1]) > 1)
    {
        for(int i = 0;i<kkCurrentPlan.agentsSize;i++)
        {
            distPointBall[i] = wm->ball->pos.dist(kkAgentList.at(i).pos);
        }
        kkGetCostRec2(distPointBall,kkCurrentPlan.agentsSize, tempListValue,
                      tempValue, kkCurrentPlan.agentsSize, 0);
    }
    else if ((twoPointSkill.size()) &&
             cntRobotInRegion(kkARegion[kkCurrentPlan.
                              agents[twoPointSkill.at(0)].B - 1]) > 1) {
        for(int i = 0; i < kkCurrentPlan.agentsSize; i++)
            for(int j = 0; j < kkAgentList.length(); j++)
                distPoint2[i][j] = kkARegion[kkCurrentPlan.agents[i].B - 1].
                        center().dist(kkAgentList.at(j).pos);

        kkGetCostRec(distPoint2, kkCurrentPlan.agentsSize, tempListValue,
                     tempValue, kkCurrentPlan.agentsSize, 0);
        ...
        for(int i = 0; i < kkCurrentPlan.agentsSize; i++)
            for(int j = 0; j < kkAgentList.length(); j++)
                distPoint[i][j] = kkARegion[kkCurrentPlan.agents[i].A - 1].
                        center().dist(kkAgentList.at(j).pos);

        kkGetCostRec(distPoint, kkCurrentPlan.agentsSize, tempListValue,
                     tempValue, kkCurrentPlan.agentsSize, 0);
...
void CForceStart::kkAssignId()
{
    ...
    QList<kkValue> tempListValue;
    kkGetCostRec(distPoint, kkCurrentPlan.agentsSize,
                 tempListValue,
                 tempValue, kkCurrentPlan.agentsSize, 0);
...
void CForceStart::kkGetCostRec(double costArr[][5], int arrSize,
                               QList<kkValue> &valueList, kkValue value,
                               int size, int aId)
{
    if(size <= 1) {
    ...
    for(int i = 0; i < size; i++) {
        ...
        kkGetCostRec(costArr, arrSize, valueList,
                     tValue, size - 1, aId + 1);
    }
}

void CForceStart::kkGetCostRec2(double costBrr[5], int arrSize,
                                QList<kkValue> &valueList, kkValue value,
                                int size, int aId)
{
    if(size <= 1) {
    ...
    for(int i = 0; i < size; i++) {
        ...
        kkGetCostRec2(costBrr, arrSize,
                      valueList, tValue, size - 1, aId + 1);
    }
}
...
bool CForceStart::kkCheckMovePos(double x, double y)
{
    ...
    {
        if(tempCircle.intersection(kkMoveObstacle.at(i), &sol1, &sol2) > 0 ||
                tempCircle.contains(kkMoveObstacle.at(i).center()))
            return false;
    }
        ...
        for(double theta = 0; theta <= 2*_PI; theta += step/xx)
        {
            ...
            if(id == 5)
            {
                if(kkARegion[id].contains(Vector2D(xx, yy)) &&
                        !wm->field->isInOppPenaltyArea(Vector2D(xx, yy)))
                {
...
void CForceStart::kkAmirNewSearch(double x, double y, short id,
                                  Polygon2D &tPoly, double step)
{
    double tempDist[4];
    tempDist[0] = Vector2D(x, y).dist(kkARegion[id].topLeft());
    tempDist[1] = Vector2D(x, y).dist(kkARegion[id].topRight());
        ...
        for(double theta = 0; theta <= 2*_PI; theta += step/xx)
        {
            ...
            if(canKickToGoal) {
                ...
                else
                {
                    if(kkARegion[id].contains(Vector2D(xx, yy)) &&
                            tPoly.contains(Vector2D(xx, yy)) &&
                            tempPoly.contains(Vector2D(xx, yy)))
                    {
                        if(kkCheckMovePos(xx, yy))
                            return;
                    }
...
Vector2D CForceStart::kkFindBestPosForMove(int fromAgent, int regionId,
                                           bool offensive)
{
    regionId--;
    ...
    offRect.assign(kkARegion[regionId].center().x,
                   kkARegion[regionId].top(),
                   kkARegion[regionId].size().length()/2,
                   kkARegion[regionId].size().width());

    defRect.assign(kkARegion[regionId].left(),
                   kkARegion[regionId].top(),
                   kkARegion[regionId].size().length()/2,
                   kkARegion[regionId].size().width());
...
bool CForceStart::kkFindBestRegionsToPass(int regionIndex, double reachTime,
                                          Polygon2D &tPoly, int tException,
                                          bool regionException)
{
    if(regionException) {
        return true;
    }
    ...
    for(int i = 0; i < 4; i++)
    {
        for(int j = i; j < 4; j++)
        {
            verticeAngles[i][j] = Vector2D::angleOf(rectVertice[i],
                                                    tempBallPos,
                                                    rectVertice[j]).abs();
        }
    }
    ...
    Line2D tempLine(rectVertice[farthestPoint],
                    (rectVertice[aPoint] - rectVertice[bPoint]).dir());
    ...
    kkInsertPassObstacleToList(reachTime, tempPoly,
                               Segment2D(tempBallPos, tempLPoint),
                               Segment2D(tempBallPos, tempRPoint),
                               tException);
    ...
    for(int i = 0; i < kkPassObstacle.length(); i++)
        blockedSegs.append(kkBlockedByRobots(kkPassObstacle.at(i),
                                             tempBallPos,
                                             tempLine));
    ...
    kkInvertSegments(blockedSegs,
                     Segment2D(tempRPoint, tempLPoint),
                     vectorList);
...
{
    Vector2D rectVertice[4];
    rectVertice[0] = Vector2D(_FIELD_WIDTH,_GOAL_WIDTH/2);
    //kkARegion[regionIndex].topLeft();
    rectVertice[1] = Vector2D(_FIELD_WIDTH+_GOAL_DEPTH,_GOAL_WIDTH/2);
    //kkARegion[regionIndex].topRight();
    rectVertice[2] = Vector2D(_FIELD_WIDTH+_GOAL_DEPTH,-_GOAL_WIDTH/2);
    //kkARegion[regionIndex].bottomRight();
    rectVertice[3] = Vector2D(_FIELD_WIDTH,-_GOAL_WIDTH/2);
    //_kkARegion[regionIndex].bottomLeft();
    ...
    kkInsertPassObstacleToList(.0, tempPoly,
                               Segment2D(passPos, tempLPoint),
                               Segment2D(passPos, tempRPoint), -1);
...
void CForceStart::kkMergeSegments(QList<kk2Segment2D> &segList)
{
    ...
    for(int i = 0; i < segList.length(); i++)
    {
        for(int j = i; j < segList.length(); j++)
        {
            ...
            {
                segList.replace(i,
                                kkMergeTwoSegments(segList.at(i), segList.at(j)));
                segList.removeAt(j);
                check = true;
                break;
            }
...
void CForceStart::kkInvertSegments(QList<kk2Segment2D> &segList,
                                   Segment2D tSeg,
                                   QList<kk2Vector2D> &vecList)
{
    ...
    if(vecList.length() >= 2)
    {
        ...
        if(Segment2D(vecList.at(vecList.length() - 2).vec1,
                     vecList.at(vecList.length() - 1).vec1).contains(tSeg.b()))
            checkB = true;
    }
...
kk2Segment2D CForceStart::kkBlockedByRobots(Circle2D tRobot,
                                            Vector2D tOrigin,
                                            Line2D tLine)
{
    kk2Segment2D temp2Seg;
...
void CForceStart::kkInsertPassObstacleToList(double reachTime,
                                             Polygon2D mPoly,
                                             Segment2D lLine,
                                             Segment2D rLine,
                                             int tException)
{
    ...
    for(int i = 0; i < wm->our.activeAgentsCount(); i++)
    {
        ...
        {
            if(oppCircle.intersection(lLine, &sol1, &sol2) > 0 ||
                    oppCircle.intersection(rLine, &sol1, &sol2))
            {
                kkPassObstacle.append(oppCircle);
            }
        }
...
Vector2D CForceStart::kkFindBestPosForPass(int fromAgent, int regionId,
                                           bool offensive, double reachTime,
                                           int tException)
{
    regionId--;
    if(regionId < 0) regionId = 0;
    ...
    kkInsertObstacleToList(regionId);
    Rect2D offRect, defRect;
    offRect.assign(kkARegion[regionId].center().x, kkARegion[regionId].top(),
                   kkARegion[regionId].size().length()/2,
                   kkARegion[regionId].size().width());

    defRect.assign(kkARegion[regionId].left(), kkARegion[regionId].top(),
                   kkARegion[regionId].size().length()/2,
                   kkARegion[regionId].size().width());

    Polygon2D tempPoly;
    bool tBool = false;
    bool canKick = kkFindBestRegionsToPass(regionId, reachTime,
                                           tempPoly, tException,
                                           tBool);
    ...
    if(offensive)
        ...
        kkBestMovePos = Vector2D(defRect.center());
    if(canKick)
        kkAmirSearch(kkBestMovePos.x, kkBestMovePos.y,
                     regionId, tempPoly, 0.08);
...
Vector2D CForceStart::kkNewFindBestPosForPass(int fromAgent, int regionId,
                                              int regionIdPasser, bool offensive,
                                              double reachTime, int tException)
{
    regionId--;
    bool hardcore = 1;
    if(regionId < 0) regionId = 0;
    ...
    pointInRect[0][0] = (kkARegion[regionId].center()
                         + kkARegion[regionId].topLeft())/2;
    pointInRect[0][1] = (kkARegion[regionId].center()
                         + Vector2D(kkARegion[regionId].left(),kkARegion[regionId].center().y))/2;
    pointInRect[0][2] = (kkARegion[regionId].center()
                         + kkARegion[regionId].bottomLeft())/2;
    pointInRect[1][0] = (kkARegion[regionId].center()
                         + Vector2D(kkARegion[regionId].center().x,kkARegion[regionId].top()))/2;
    pointInRect[1][1] = (kkARegion[regionId].center())/2;

    pointInRect[1][2] = (kkARegion[regionId].center()
                         + Vector2D(kkARegion[regionId].center().x,kkARegion[regionId].bottom()))/2;
    pointInRect[2][0] = (kkARegion[regionId].center()
                         + Vector2D(kkARegion[regionId].topRight()))/2;
    pointInRect[2][1] = (kkARegion[regionId].center()
                         + Vector2D(kkARegion[regionId].right(),kkARegion[regionId].center().y))/2;
    pointInRect[2][2] = (kkARegion[regionId].center()
                         + Vector2D(kkARegion[regionId].bottomRight()))/2;
...
void CForceStart::kkNewAssignTasks()
{
    for(int i = 0; i < 5; i++)
        kkActiveAgent[i] = false;
    int receiveIndex = -1;
    //////////////////Check There is Any Receiver Or OneToucher
    for(int i = 0; i < kkCurrentPlan.agentsSize; i++)
    {
        if(kkHasReceiveOrOneTouch(i))
        {
...
void CForceStart::kkManToManMarkTwo(int ids[]){
    ...
    for(int ii = 0; ii < kkActiveAgents.count(); ii++)
    {
        tempIDs.append(kkActiveAgents.at(ii)->id());
    }
    for(int ii = kkActiveAgents.count(); ii < wm->our.activeAgentsCount(); ii++)
    {
        tempIDs.append(-1);
    }
    ...
    {
        for(int jj = 0; jj < wm->our.activeAgentsCount(); jj++)
        {
            if(tempIDs.at(jj) == wm->our.active(jj)->id)
                continue;
            if(Circle2D(wm->opp.active(ii)->pos, 0.3).intersection(Circle2D(wm->our.active(jj)->pos, 0.15), &sol1, &sol2))
            {
                OppW[ii][0] = 0;
            }
        }
    ...
    Vector2D vctmp[2];
    Circle2D(OppPos[(int)OppW[0][1]],0.20).intersection(Line2D(OppPos[(int)OppW[0][1]],wm->field->ourGoal()),&vctmp[0],&vctmp[1]);
    if(wm->field->ourGoal().dist(vctmp[0])<wm->field->ourGoal().dist(vctmp[1]))
        defPos[0]=vctmp[0];
    else
        defPos[0]=vctmp[1];

    Circle2D(OppPos[(int)OppW[1][1]],0.20).intersection(Line2D(OppPos[(int)OppW[1][1]],wm->field->ourGoal()),&vctmp[0],&vctmp[1]);
    if(wm->field->ourGoal().dist(vctmp[0])<wm->field->ourGoal().dist(vctmp[1]))
        defPos[1]=vctmp[0];
    else
        defPos[1]=vctmp[1];
    ...
    for(int i = 0; i < 2; i++)
    {
        ...
        debug(QString("ids : %1 ,%2").arg(ids[0]).arg(ids[1]),D_KK);
...
void CForceStart::kkManToManMarkThree(int ids[]){
    ...
    Circle2D(OppPos[(int)OppW[2][1]],0.20).intersection(Line2D(OppPos[(int)OppW[2][1]],wm->field->ourGoal()),&vctmp[0],&vctmp[1]);
    if(wm->field->ourGoal().dist(vctmp[0])<wm->field->ourGoal().dist(vctmp[1]))
        defPos[2]=vctmp[0];
    else
        defPos[2]=vctmp[1];
    ...
    {
        Circle2D(OppPos[(int)OppW[0][1]],0.20).intersection(Line2D(OppPos[(int)OppW[0][1]],ballPos),&vctmp[0],&vctmp[1]);
    ...
    {
        Circle2D(OppPos[(int)OppW[1][1]],0.20).intersection(Line2D(OppPos[(int)OppW[1][1]],ballPos),&vctmp[0],&vctmp[1]);
        if(ballPos.dist(vctmp[0])<ballPos.dist(vctmp[1]))
            defPos[1]=vctmp[0];
        else
            defPos[1]=vctmp[1];
    ...
    {
        Circle2D(OppPos[(int)OppW[2][1]],0.20).intersection(Line2D(OppPos[(int)OppW[2][1]],ballPos),&vctmp[0],&vctmp[1]);
        if(ballPos.dist(vctmp[0])<ballPos.dist(vctmp[1]))
            defPos[2]=vctmp[0];
        else
            defPos[2]=vctmp[1];
...
void CForceStart::kkManToManMark(int id){
    ...
    for(i=0,j=0;i<wm->opp.activeAgentsCount();i++)
    {
        ...
        if(/*wm->opp.active(i)->markedBif(AHZTime > 100){
                            debug(QString("Time out / failed"),D_SEPEHR);
                            return true;
                        }yDefense !=1 &if(AHZTime > 100){
                            debug(QString("Time out / failed"),D_SEPEHR);
                            return true;
                        } wm->opp.active(i)->markedByMark !=1*/ 1 )
        {
            OppPos[j]=wm->opp.active(i)->pos;
            OppTh[j]=1-(Vector2D::angleBetween(OppPos[j],wm->field->ourGoal()).degree()/110);
            OppDistToBall[j]=1/(1+abs(OppPos[j].dist(ballPos)-1));
    ...
    Circle2D(OppPos[(int)OppW[0][1]],0.50).intersection(Line2D(OppPos[(int)OppW[0][1]],wm->field->ourGoal()),&vctmp[0],&vctmp[1]);
    if(wm->field->ourGoal().dist(vctmp[0])<wm->field->ourGoal().dist(vctmp[1]))
        defPos[0]=vctmp[0];
    else
        defPos[0]=vctmp[1];
...
int CForceStart::getSymmetry(int _Region) {
...
/////////////////////MAHI's Debug///////////////////
void CForceStart::mahiDebugs() {
    if(agentSkill[0] != NULL)
        debug(QString("Agent1Task : %1").arg(agentSkill[0]->getSelectedSkill()),D_MAHI);
    if(agentSkill[1] != NULL)
        debug(QString("Agent2Task : %1").arg(agentSkill[1]->getSelectedSkill()),D_MAHI);
    debug(QString("SKILL : %1").arg(kkCurrentPlan.agents[0].skill[0]),D_MAHI);
    debug(QString(kkCurrentPlan.comment),D_MAHI);
    debug(QString("First %1").arg(debugs[0]),D_MAHI);
    debug(QString("second %1").arg(debugs[1]),D_MAHI);

}

//////////////////////////////////////////////////////
bool CForceStart::kkIsPlanMatch(QList<kkRobot> aList, kkAgentPlan plan) {
    int matchCnt = 0;
    for(size_t i = 0;i < plan.agentsSize;i++) {
        for(size_t j = 0;j < aList.size();j++) {
            if(kkARegion[plan.agents[i].A].contains(aList.at(j).pos)) {
                matchCnt++;
                break;
            }
        }
    }
    if(matchCnt == plan.agentsSize) {
        return true;
    }
    return false;
}
