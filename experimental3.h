...
void CMainApplication::Experimental3()
{
    ...
    //            cycleCnt++;
    //            if (cycleCnt > 20) cycleCnt = 5;
    //            newCycle = false;
    //        }
    //    } else {
    //        newCycle = true;
    //        if (ballposes.size() < cycleCnt) {
    //            ballposes.append(qMakePair(counter,wm->ball->vel.y - 4));
    //            counter++;
    //        }
    //    }
    //    mainCnt++;
    //    knowledge->plotWidgetCustom[1] = wm->ball->vel.y - 4;
    //    if(ballposes.length() >= cycleCnt){
    //        reg.fitToDataSet(ballposes,2);
    //        knowledge->plotWidgetCustom[3]=reg.val(mainCnt+10);
    //    } else {
    //        knowledge->plotWidgetCustom[3]=-4;
    ...
    if(wm->ball->vel.y < 0.1){
        ...
        for(int i=0;i<ballposes.size();i+=2){
            draw(ballposes.at(i));
            if(i>5)
                break;
        }
        for(int i=1;i<ballposes.size();i+=2){
            draw(ballposes.at(i) , 0 , QColor(Qt::red));
            if(i>5)
                break;
        }
        ...
        debug(QString("diff:%1").arg(wm->ball->pos.y-prev_Ball_pos.at(0)),D_NADIA);
    ...
    //        if(ballposes.length()>10){
    //            ballposes.pop_front();

    //        }
    //        ballposes.append(wm->ball->vel);

    ////        debug(QString("vel:%1__").arg(ballposes.at(indexN).length()),D_NADIA);
    //        indexN++;
    //        if(indexN>10)
    //            indexN=10;
    //    }
    ...
    //    if(ballposes.length()>=10){
    ////        for(int i=0;i<10;i++){
    ////            nnn+=ballposes.at(i).length();
    ////        }
    //        nnn=ballposes.at(9).length()-ballposes.at(0).length();
    //        nnn/=10;
    //        if(flagN)
    //            debug(QString("bALL vel:%1").arg(nnn),D_NADIA);
    //        flagN=false;
    //        analyze("vel",wm->ball->vel.length(),true);
    //        analyze("miangin",nnn,true);
    //    }
    //    if(wm->ball->vel.length()<0.3){
    //        ballposes.clear();
    //        flagN=true;
