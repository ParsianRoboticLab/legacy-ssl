...
void CMainApplication::Experimental2()
{
    ...
    draw(Segment2D(wm->field->oppGoalL() /*+ Vector2D(+xDiff,+goalLineExtra)*/,
                   wm->field->oppGoalR() /*+ Vector2D(+xDiff,-goalLineExtra)*/), "yellow");
    ...
    //    debug(QString("angularVel: %1").arg(wm->opp[1]->angularVel), D_FATEMEH);


    //    draw(wm->field->oppGoalL(),0,"yellow");
