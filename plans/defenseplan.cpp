...
DefensePlan::DefensePlan()
{
    ...
    ballIsBehindGoalKeeper = false, goalieOneTouch = false,goalieClearMode = false, ballIsOutOfField = false;
    behindBallThr = 0;
...
bool DefensePlan::defenseClearOrNot(){
    ...
    if(lastClearID != -1 && ((clearFrameCnt < 30 && savedClearDist < 0.05)
        ...
        if(knowledge->goalie != NULL){
            for(int i = 0; i < min(wm->our.activeAgentsCount() , defenseAgents.count()); i++){
